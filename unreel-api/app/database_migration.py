...
def add_user_id_column():
    """
    Add the userId column to the analyses table if it doesn't exist.
    """
    try:
        # Check if the column exists
        check_column_sql = """
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_name='analyses' AND column_name='userId';
        """
        ...
        with engine.connect() as connection:
            ...
            if not column_exists:
                ...
                connection.execute(text(add_column_sql))
                connection.commit()
                logger.info("Successfully added userId column to analyses table")
            else:
                logger.info("userId column already exists in analyses table")
